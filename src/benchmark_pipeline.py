...
Usage:
    python benchmark_pipeline.py --pages 50 --output-dir benchmarks/
"""
...
try:
    ...
    from pdfplumber_tess_extractor import PDFTableExtractor
    from aws_textract import AWSTextractExtractor
    EXTRACTORS_AVAILABLE = True
except ImportError as e:
    print(f"Warning: Some extractors not available: {e}")
...
class PerformanceBenchmarker:
    ...
    def benchmark_pdfplumber(self, pdf_path: str) -> BenchmarkResult:
        ...
        try:
            ...
            # Extract metrics from result
            success = result is not None and 'tables' in result
            tables_count = len(result.get('tables', [])) if success else 0
            pages = result.get('total_pages', 0) if success else 0
